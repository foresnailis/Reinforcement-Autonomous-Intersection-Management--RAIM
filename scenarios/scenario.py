    ...
    def _get_intervals(self):
        for i in range(0, len(self.args), self.gpt*3):
            ...
            if self.gpt == 1:
                ...
                endWE = group[13]
                probWE = group[14]
                beginWE_ped = group[15]
                endWE_ped = group[16]
                probWE_ped = group[17]
                beginEW = group[18]
                endEW = group[19]
                probEW = group[20]
                beginEW_ped = group[21]
                endEW_ped = group[22]
                probEW_ped = group[23]
                # 车辆右转、行人左转
                beginNW = group[24]
                endNW = group[25]
                probNW = group[26]
                beginNW_ped = group[27]
                endNW_ped = group[28]
                probNW_ped = group[29]
                beginSE = group[30]
                endSE = group[31]
                probSE = group[32]
                beginSE_ped = group[33]
                endSE_ped = group[34]
                probSE_ped = group[35]
                beginWS = group[36]
                endWS = group[37]
                probWS = group[38]
                beginWS_ped = group[39]
                endWS_ped = group[40]
                probWS_ped = group[41]
                beginEN = group[42]
                endEN = group[43]
                probEN = group[44]
                beginEN_ped = group[45]
                endEN_ped = group[46]
                probEN_ped = group[47]
                # 左转
                beginNE_ped = group[48]
                endNE_ped = group[49]
                probNE_ped = group[50]
                beginES_ped = group[51]
                endES_ped = group[52]
                probES_ped = group[53]
                beginSW_ped = group[54]
                endSW_ped = group[55]
                probSW_ped = group[56]
                beginWN_ped = group[57]
                endWN_ped = group[58]
                probWN_ped = group[59]
    ...
    def _create_flows(self):
        ...
        for beginNS, endNS, probNS, beginNS_ped, endNS_ped, probNS_ped, \
            ...
            for i in range(1, self.sg.cols-1):
                ...
                routeid = routs('NS_ped', i)
                self.add_flow(Flow(flows('NS_ped_', i, j), beginNS_ped, endNS_ped,
                              vehsPerHour=probNS_ped, route=routeid, tpe='pedestrian'))

                routeid = routs('SN', i)
                self.add_flow(Flow(flows('SN', i, j), beginSN, endSN, vehsPerHour=probSN,
                              route=routeid, departSpeed='max', tpe='typedist1'))

                routeid = routs('SN_ped', i)
                self.add_flow(Flow(flows('SN_ped_', i, j), beginSN_ped, endSN_ped,
                              vehsPerHour=probSN_ped, route=routeid, tpe='pedestrian'))
            # 东西走向车流
            for i in range(1, self.sg.rows-1):
                routeid = routs('WE', i)
                self.add_flow(Flow(flows('WE', i, j), beginWE, endWE, vehsPerHour=probWE,
                              route=routeid, departSpeed='max', tpe='typedist1'))

                routeid = routs('EW', i)
                self.add_flow(Flow(flows('EW', i, j), beginEW, endEW, vehsPerHour=probEW,
                              route=routeid, departSpeed='max', tpe='typedist1'))
